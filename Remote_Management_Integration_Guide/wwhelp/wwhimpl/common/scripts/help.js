...
function  WWHHelp_Object(ParamURL)
{
  ...
  // VERITAS change, make sure locale is fixed from User Macros.
  //this.mLocale         = Vx_Locale;
...
function  WWHHelp_DetermineContextDocument()
{
  ...
  else if (URLParams[0] != null)  // context specified
  {
    ...
    {
      ContextDocumentURL = this.mBaseURL + "00000000.html";
    }
  }
  else  // Display splash page if nothing else found
    {
...
function  WWHHelp_GetSyncPrevNext(ParamHREF)
{
  ...
  {
    if (BookFileHREF == "00000000.html")
    {
      ResultArray[2] = this.mBooks.fBookFileIndiciesToHREF(0, 1);
    }
    else
    {
  ...
  {
    // Veritas modification for multi-book sets
    ResultArray = this.mBooks.fVxGetSyncPrevNext(AbsoluteHREF);
  }
...
function  WWHHelp_HREFToTitle(ParamHREF)
{
  ...
  if (BookFileHREF == null)
  {
     Parts = this.mBooks.fVxGetBookIndexFileHREF(AbsoluteHREF);
     Parts[1] = Parts[2];
  }
  else
  {
...
//alert('In HREFtoTitle; ParamHREF='+ParamHREF+'; BookFileHREF='+BookFileHREF);
  if (Parts[0] >= 0)
  {
  ...
  // Veritas modification to display ">" correctly in browser titles.
...
function  WWHHelp_Update(ParamURL)
{
 ...
 else if (ParamURL.indexOf("00000000.html") == -1)
  {
...
// Update window title, if possible
  //
  if (ParamURL.indexOf("00000000.html") == -1)
  {
